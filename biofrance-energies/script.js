...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function isCurrentStepValid() {
    ...
    for (const field of fields) {
      // si pas required → on ne teste pas
  ...
  /* ---------- 2) Données & fonctions pour le calcul CEE ---------- */
  ...
  function runPacSimulation() {
    ...
    if (recapContent) {
      ...
      lignes.push('');
      lignes.push(`Reste à charge estimé (après primes CEE) : ${rac.toLocaleString('fr-FR')} € TTC`);
      recapContent.textContent = lignes.join('\n');
    }
  }

  /* ---------- 3) Clics sur Suivant / Retour ---------- */
